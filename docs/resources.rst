...
or a similar form of data storage. In BoxMeAPI, these resources are generally
...
A sample resource definition might look something like::
    ...
    from bmga.authorization import DjangoAuthorization
    from bmga.resources import ModelResource, ALL, ALL_WITH_RELATIONS
...
your needs. APIs are rarely a one-size-fits-all problem space, so BoxMeAPI
tries to get the fundamentals right and provide you with enough hooks to
customize things to work your way.

As is standard, this raises potential problems for thread-safety. BoxMeAPI has
...
BoxMeAPI can be thought of as a set of class-based views that provide the API
...
* ``get_list`` does the actual work of the API. It does:
  ...
  * The objects in the page have ``full_dehydrate`` applied to each of them,
    causing BoxMeAPI to translate the raw object data into the fields the
    endpoint supports.
...
Resource URIs play a heavy role in how BoxMeAPI delivers data. This can seem
very different from other solutions which simply inline related data. Though
BoxMeAPI can inline data like that (using ``full=True`` on the field with the
...
data model on the server side. For this, BoxMeAPI has the "dehydrate/hydrate"
cycle.

The Dehydrate Cycle
-------------------

BoxMeAPI uses a "dehydrate" cycle to prepare data for serialization, which is
...
BoxMeAPI uses a "hydrate" cycle to take serialized data from the client
and turn it into something the data model can use. This is the reverse process
from the ``dehydrate`` cycle. In fact, by default, BoxMeAPI's serialized data
...
Unlike Django's ORM, BoxMeAPI does not automatically create reverse relations.
...
.. warning::
  ...
  even if it's in the same module. BoxMeAPI (intentionally) lacks a construct
  like the ``AppCache`` which makes that sort of thing work in Django. Sorry.

BoxMeAPI also supports self-referential relations. If you assume we added the
appropriate self-referential ``ForeignKey`` to the ``Note`` model, implementing
a similar relation in BoxMeAPI would look like::
...
:class:`~bmga.resources.ModelResource` provides a basic Django ORM filter
...
class::
    ...
    from bmga.constants import ALL, ALL_WITH_RELATIONS
...
:class:`~bmga.resources.Resource` may define a custom
:meth:`~bmga.resource.Resource.build_filters` method which allows you to
...
Largely relies on ``bmga.utils.mime.determine_format`` but here
