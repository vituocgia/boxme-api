...
``Meta`` option to the ``Resource`` in question. For example::
    ...
    from bmga.authentication import BasicAuthentication
...
BoxMeAPI ships with the following ``Authentication`` classes:

.. warning:

    BoxMeAPI, when used with ``django.contrib.auth.models.User``, will check
...
machine-generated api key. BoxMeAPI ships with a special ``Model`` just for
this purpose, so you'll need to ensure ``bmga`` is in ``INSTALLED_APPS`` and 
...
BoxMeAPI includes a signal function you can use to auto-create ``ApiKey``
objects. Hooking it up looks like::

    from django.contrib.auth.models import User
    from django.db.models import signals
    from bmga.models import create_api_key
...
.. _`the documentation for this setting`: https://boxme-api.readthedocs.io/en/latest/settings.html#bmga-abstract-apikey
...
machine-generated api key. As with ApiKeyAuthentication, ``bmga``
...
attempting each until successfully authenticating. For example::
    ...
    from django.contrib.auth.models import User
    from bmga.authentication import BasicAuthentication, ApiKeyAuthentication, MultiAuthentication
...
optional but recommended to be customized)::
    ...
    from bmga.authentication import Authentication
