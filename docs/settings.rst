...
with ``BMGA_CANNED_ERROR``) in the response.

An example::

    BMGA_FULL_DEBUG = True
...
An example::
    ...
    BMGA_DEFAULT_FORMATS = ['json', 'xml']

Defaults to ``['json', 'xml', 'yaml', 'plist']``.


``BMGA_ABSTRACT_APIKEY``
============================
