...
We'll assume that we're interacting with the following BoxMeAPI code::
...
BoxMeAPI tries to treat all clients & all serialization types as equally as
...
all interactions with BoxMeAPI through relatively few endpoints.
...
type of request to make), we'll start there. BoxMeAPI tries to expose various
...
nor do you want to do multiple requests. BoxMeAPI includes a "set" view, which
...
BoxMeAPI also gives you full write capabilities in the API. Since the
...
.. warning::
  ...
  This is why ``ReadOnlyAuthorization`` is the default in BoxMeAPI & why you
...
.. note::
    ...
    BoxMeAPI encourages "round-trippable" data, which means the data you
    ...
    another object & see what BoxMeAPI thinks it should look like.
...
As with all BoxMeAPI requests, the headers we request are important. Since
...
The ``Content-Type`` header here informs BoxMeAPI that we're sending it JSON.
...
That's a whirlwind tour of interacting with a BoxMeAPI API. There's additional
