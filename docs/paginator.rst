...
Similar to Django's ``Paginator``, BoxMeAPI includes a ``Paginator`` object
...
works. ``limit`` & ``offset`` (bmga) are used in place of ``page``
...
by BoxMeAPI. Like the others, you provide the ``Paginator`` (or your own
...
Another common request is to alter the structure BoxMeAPI uses in the
