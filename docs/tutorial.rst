...
=============================
Getting Started with BoxMeAPI
=============================

BoxMeAPI is a reusable app (that is, it relies only on its own code and
...
Not everyone's needs are the same, so BoxMeAPI goes out of its way to provide
plenty of hooks for overriding or extending how it works.

.. note::

    If you hit a stumbling block, you can join
    `#bmga on irc.freenode.net`_ to get help.

.. _#bmga on irc.freenode.net: irc://irc.freenode.net/bmga
...
With that, we'll move on to installing and configuring BoxMeAPI.


Installation
============

Installing BoxMeAPI is as simple as checking out the source and adding it to
your project or ``PYTHONPATH``.
  ...
  2. Either check out bmga from GitHub_ or to pull a release off PyPI_.
     Doing ``sudo pip install boxme-api`` or
     ``sudo easy_install boxme-api`` is all that should be required.
  3. Either symlink the ``bmga`` directory into your project or copy the
     directory in. What ever works best for you.

.. _GitHub: https://github.com/vituocgia/boxme-api
.. _PyPI: http://pypi.python.org/pypi/boxme-api


Configuration
=============

The only mandatory configuration is adding ``'bmga'`` to your
``INSTALLED_APPS``. This isn't strictly necessary, as BoxMeAPI has only two
...
with BoxMeAPI involves creating :class:`~bmga.resources.Resource` classes.
...
This class, by virtue of being a :class:`~bmga.resources.ModelResource`
subclass, will introspect all non-relational fields on the ``Entry`` model and
create its own :mod:`ApiFields <bmga.fields>` that map to those fields,
...
.. note::
    ...
    instances of :class:`~bmga.resources.Resource` and lowercasing the
...
.. note::
    ...
    in the browser (accept headers vary between browsers). BoxMeAPI properly
    ...
    BoxMeAPI lets you specify ``?format=...`` when you really want to force
    a certain type.
...
getting "401 Unauthorized" errors. For safety, BoxMeAPI ships with the
...
BoxMeAPI does **NOT** introspect related data because it has no way to know
...
``myapp/api.py`` to match the following code::
    ...
    class EntryResource(ModelResource):
        # Maps `Entry.user` to a BoxMeAPI `ForeignKey` field named `user`,
        # which gets serialized using `UserResource`. The first appearance of
        # 'user' on the next line of code is the BoxMeAPI field name, the 2nd
...
We simply created a new :class:`~bmga.resources.ModelResource` subclass
...
BoxMeAPI ships with an :class:`~bmga.api.Api` class, which lets you bind
multiple :class:`Resources <bmga.resources.Resource>` together to form a
...
Note that we're now creating an :class:`~bmga.api.Api` instance,
...
We now have a full working API for our application. But BoxMeAPI supports many
more features, like:
...
BoxMeAPI is also very easy to override and extend. For some common patterns and
