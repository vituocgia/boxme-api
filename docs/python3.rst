...
As of BoxMeAPI v0.10.0, it has been ported to support both Python 2 & Python 3
within the same codebase. This builds on top of what `six`_ & `Django`_ provide.

No changes are required for anyone running an existing BoxMeAPI
...
BoxMeAPI was depending on several Oauth libraries for that authentication
...
To mitigate this, BoxMeAPI now tries to ensure that serialized data is sorted
