...
from bmga.exceptions import BadRequest
from bmga.serializers import _get_default_formats, Serializer
...
class GetDefaultFormatsTestCase(TestCase):
    @mock.patch.multiple('bmga.serializers', lxml=True, yaml=True, biplist=True)
    def test_all(self):
        formats = _get_default_formats()

        self.assertEqual(formats, ['json', 'xml', 'yaml', 'plist'])

    @mock.patch.multiple('bmga.serializers', lxml=False, yaml=False, biplist=False)
